    ...
    internal class FrontEventProcessor : IProcessor
    {
        ...
        private void RegisterEventHandler(Type handlerType)
        {
            ...
            foreach(var handlerInterface in handlerInterfaces)
            {
                ...
                if (string.IsNullOrWhiteSpace(eventName))
                    throw new Exception($"Для {handlerInterface.Name}({eventType.Name}) d {handlerType.Name}  не заданно имя Event (SalEventNameAttribute)");
        ...
        public Task ExecuteEventHandlerAsync(EventHandlerInfo ehi, TransportMessage transportMessage, EventPayload eventPayload, ILogger eventLogger)
        {
            ...
            if (ehi.IsCommon)
            {
                ...
                {
                    return eha.Handle(eventPayload.Payload.Clone(), context, executingContext);
                }
                else
                {
